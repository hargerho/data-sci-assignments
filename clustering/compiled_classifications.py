...
def silhouetteVisualizer(lst, title):
    ...
    # Printing the results rounded to 3sf
    roundedList = [round(value, 3) for value in lst]
    print("{} silhouette scores: {}".format(title, roundedList))
...
def initializeCentroids(data, k):
    ...
    for iterator in range(k - 1):
        ...
        sumDist = sum(dist)
        for candidate in range(len(dist)):
            dist[candidate] /= sumDist

        # Datapoint with the highest probability is the next centroid
...
def KMeans(data, k, maxIter, plusFlag):
    ...
    for i in range(maxIter):
        ...
        # Assigning the datapoints to the clusters
        ...
        # Terminates when the centroid is the same
        else:
            break
...
def intraCalculator(data, distFunction, clustering):
    ...
    # Computes the distance of each datapoint to every other datapoint
    # Arranged by data index
...
def bisectKMeans(data, s, maxIter, plusFlag):
    ...
    # If want more than 2 clusters
        ...
        while len(clusterDict) != s:
            ...
            # Find the highest key number in clusterDict
            highestKey = max(clusterDict.keys())

            # Create a new dictionary with renamed keys from tmpDict
